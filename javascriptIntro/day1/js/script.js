// console.log("Hello");
// console.log("Hello Again");
// alert("this is an alert");
// console.log("after alert");

// console.log("this is a string");
// console.log(10);
// console.log("10")

// console.log(1 * 2);


// console.log("string 1" + " string 2")

// var variable1 = "this is now a string";
// console.log(variable1);
// variable1 += "something else";
// console.log(variable1);

// var question = prompt("What is your name?");
// var secondQuestion = prompt("What is your fav colour?")
// console.log(question);
// console.log(secondQuestion);
// console.log("Hello "+question+" your fav colour is "+secondQuestion);


// document.write("<h2 class='red'>"+question+"</h2>");


var question1 = prompt("What is your name?");
// document.write("<h2>What is your name?</h2>");
// document.getElementById("pageContainer").innerHTML = "<h2>What is your name?</h2>";
// document.getElementById("pageContainer").innerHTML += "<h3>"+question1+"</h3>";
document.getElementById("heading").innerText = "Hello "+ question1;
document.getElementById("yourName").innerText += question1;
// document.write("<h3>"+question1+"<h3>");
//
var question2 = prompt("What is your favorite TV show?");
// document.write("<h2>What is your favorite TV show?</h2>");
document.getElementById("favShow").innerText += question2; 
// document.write("<h3>"+question2+"<h3>");
//
var question3 = prompt("What is your favorite song?");
// document.write("<h2>What is your favorite song?</h2>");
document.getElementById("favSong").innerText += question3;
// document.write("<h3>"+question3+"<h3>");
//
var question4 = prompt("Who is your favorite superhero?");
// document.write("<h2>Who is your favorite superhero?</h2>");
document.getElementById("favHero").innerText += question4;
// document.write("<h3>"+question4+"<h3>");
